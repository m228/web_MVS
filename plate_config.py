...
DEFAULTS = {
    ...
    # Задано по слову Макса: Подкачка (стадия 5) — 5 с, Рост (стадия 7) — 10 с, остальные — 30 с.
    "cycle_period_s": {"5": 5, "7": 10, "default": 30},

    # ПРЕДОХРАНИТЕЛЬ шага цикла (сек): сколько ждать доезда мотора, прежде чем бросить
    # шаг по таймауту. Нормальный выход шага — «доехал» (позиция у цели), а это лишь
    # защита от вечного зависания, если мотор не едет. Ставь с запасом от реального
    # времени хода: путь / скорость платы (напр. 40мм при ~36 мкм/с ≈ 18 мин).
    "step_timeout_s": 300,

    # ежечасная промывка от засахаривания: раз в час (на минуте `minute`) открыть ОБА клапана
    # промывки на sec_on..sec_off и закрыть (~5 с). Всегда активна; идёт только в простое (mode 0).
    "hourly_wash": {"enabled": True, "minute": 3, "sec_on": 0, "sec_off": 5},

    # цикл пробы «Взять пробу» (правится на вкладке «Цикл»):
    # отвод в retract_pos -> промывка pre_wash_sec -> подвод по СВ (трубка открыта) ->
    # выдержка dwell_sec (скрины каждые shot_interval_sec + видео) -> возврат в retract_pos.
    # Доезд определяется по аналоговому датчику pos1_ai.
    "probe_cycle": {"retract_pos": 20000, "pre_wash_sec": 4, "dwell_sec": 15, "shot_interval_sec": 3},
    ...
    "camera_serial": "",
    "camera_ip": "",
    # режим съёмки: "stream" — видео постоянно; "trigger" — кадр снимается по сигналу авто-цикла
    # (soft-триггер, эндпоинт /api/camera/snap). Авто-цикл (FSM) прочитает это позже.
    "camera_mode": "stream",
    ...
    "sv_source": {
        ...
        "fields": {
            ...
            "temp_app":  {"reg": 651, "scale": 1, "signed": True},    # температура в ВА, °C    AI[1][13] (знаковый: <0 приходит как >65000)
