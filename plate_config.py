...
DEFAULTS = {
    ...
    "sv_source": {
        "enabled": False,
        "host": "",            # IP контроллера (ПЛК); пусто -> берётся host платы
        "port": 502,
        "unit": 255,
        "period_s": 1.0,
        # ПОЛЯ аппарата: name -> {reg, scale}. reg=0 -> поле не читается (показываем «(—)»).
        # инж.значение = регистр / scale (напр. СВ 82.62 при рег 8262 -> scale 100).
        # Добавляй сюда любые данные аппарата (ток циркулятора, уровень, давления и т.д.) — они
        # появятся в телеметрии и на будущей мнемосхеме вакуум-аппарата.
        # signed=true -> знаковый int16 (для давлений/разрежения — они отрицательные)
        "fields": {
            "sv":        {"reg": 0, "scale": 100},                   # СВ (Brix)
            "stage":     {"reg": 0, "scale": 1},                     # стадия варки (номер)
            "temp_app":  {"reg": 0, "scale": 10,   "signed": True},  # температура аппарата, °C
            "current":   {"reg": 0, "scale": 10},                    # ток циркулятора, A
            "level":     {"reg": 0, "scale": 100},                   # уровень утфеля, %
            "press_top": {"reg": 0, "scale": 1000, "signed": True},  # давление верх (разрежение), бар
            "press_bot": {"reg": 0, "scale": 1000, "signed": True},  # давление низ, бар
            "vacuum":    {"reg": 0, "scale": 1000, "signed": True},  # разрежение (общее), бар
        },
