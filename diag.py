...
Запуск из ИСХОДНИКОВ (в том же venv, где работает приложение):
    python diag.py

Запуск из СОБРАННОГО exe (то же frozen-окружение, что у приложения!):
    web_MVS.exe --diag
    (вывод дублируется в файл diag_output.txt рядом с exe)
...
MVS, загрузку зависимых DLL продюсера и полный traceback падений. Цель — понять,
почему из PyCharm камеры видны, а из exe enumerate даёт 0 устройств.
"""
import os
import sys
import platform
...
# Проба загрузки зависимых DLL продюсера через ctypes. Главная гипотеза «работает
# из PyCharm, но не из exe»: в замороженном процессе не грузится Hikrobot-рантайм
# продюсера (GenApi_*_MV / GCBase_*_MV / MvCameraControl / MvRender). Если тут
# что-то не грузится — вот и причина пустого списка устройств.
def _probe_runtime_dlls(cti_dir):
    print("== ПРОБА ЗАГРУЗКИ DLL ПРОДЮСЕРА (ctypes) ==")
    print("папка продюсера:", cti_dir)
    try:
        import ctypes
    except Exception as exc:
        print("  ctypes недоступен:", exc)
        return
    # добавим папку продюсера в поиск DLL (как это делает приложение)
    try:
        if hasattr(os, "add_dll_directory") and Path(cti_dir).is_dir():
            os.add_dll_directory(str(cti_dir))
    except Exception as exc:
        print("  add_dll_directory не удалось:", exc)

    patterns = ["MvCameraControl.dll", "GenApi_*.dll", "GCBase_*.dll",
                "Log_*.dll", "MvRender*.dll", "MVGigEVisionSDK*.dll",
                "MvProducerGEV.cti"]
    seen = set()
    cti_dir = Path(cti_dir)
    for pat in patterns:
    ...
    if not seen:
        print("  рядом с .cti нет ни одной ожидаемой DLL — рантайм продюсера в другом месте")
...
def main():
    ...
    for var in ("MVS_CTI_PATH", "GENICAM_GENTL64_PATH",
                ...
                "MVCAM_COMMON_RUNENV", "MVCAM_SDK_PATH", "PATH"):
        val = os.environ.get(var)
        if var == "PATH" and val:
            val = "\n      " + "\n      ".join(val.split(os.pathsep))
        print(f"  {var} = {val}")
    ...
    line()
    _probe_runtime_dlls(Path(cti).parent)

    line()
    print("== ПЕРЕЧИСЛЕНИЕ + СОЗДАНИЕ ==")
    from harvesters.core import Harvester

    # логгер harvesters на DEBUG — покажет, что происходит при enumerate внутри
    # (часто ловит немую причину, почему устройств 0)
    hlog = logging.getLogger("harvesters")
    hlog.setLevel(logging.DEBUG)
    handler = logging.StreamHandler(sys.stdout)
    handler.setFormatter(logging.Formatter("  [harvesters] %(levelname)s %(message)s"))
    hlog.addHandler(handler)

    h = Harvester(logger=hlog)
    h.add_file(str(cti))
    h.update()
    ...
    # падало -1003 на register_event. С compat-патчем должно проходить.
    # ----------------------------------------------------------------------
    line()
    print("== STAGE 2: повторный update() + create() (проверка compat-патча) ==")
    import gc
    gc.collect()
    h2 = Harvester(logger=hlog)
    h2.add_file(str(cti))
    h2.update()
    ...
    # ----------------------------------------------------------------------
    # STAGE 3: реальный путь приложения через manager.open_node_map().
    # ----------------------------------------------------------------------
    line()
